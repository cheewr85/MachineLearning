...
- Predictive Power
  ...
  - 데이터세트가 작을수록, 데이터의 예시 속에서 label과 관련된 feature를 찾을 확률이 높음
  - 이론적 뒷받침없이 많은 feature를 시도한다면 모델의 잘못된 연관성을 믿게 됨 / 모델이 예측하고 일반화시키기 전까지는 깨닫지 못함
- Predictions vs. Decisions
  - 예측이 아닌 결과에 대해서 초점을 맞춰라
  <img src="https://user-images.githubusercontent.com/32586985/72671296-65bd6e80-3a8b-11ea-8c4a-b1869b6cde94.PNG">
  
### 어려운 ML 문제
- Clustering
  - 각각의 cluster가 unsupervised learning problem에서 어떤 의미를 가지고 있을까? / 예를 들어 모델이 유저가 blue cluster안에 있다고 한다면 blue cluster가 나타내는 것이 무엇인지 알려고 할 것임
  - 가끔씩 clustering을 기준으로 결과를 도출하는 것 예를 들어 clustering을 이용하여 같은 사람의 사진을 그룹화하는 것을 예로 들 수 있음
  - 다른 한편으론 cluster을 기준으로 결과를 취하는 것이 어려울 수 있음
  - cluster의 의미를 찾으려고 하지만 이것은 잘못된 것일 수 있는데 왜냐하면 모델이 당신의 직관이 담긴 기준을 그룹화를 하지 않을 수 있기 때문
  - 하나의 대안적인 접근법은 cluster을 하기전에 전체 cluster를 넘어 이러한 라벨들에 전파시키는 것이 있음
  - 예를 들어 모든 라벨 X의 아이템이 하나의 cluster로 귀결되면 다른 예시들도 라벨 X로 확장시킬 수 있음
<img src="https://user-images.githubusercontent.com/32586985/72671392-d022de80-3a8c-11ea-9825-200cdf80c18d.PNG">

- Anomaly Detection
  - 가끔씩 ML을 통해서 anomaly를 규정하길 원하지만 라벨이 있는 데이터에서 얻는 것이 anomaly로 무엇이 구성되었는지 어떻게 결정한 것인지 알기 힘듬
  - 하나의 선택지는 휴리스틱을 정의하거나 anomalies의 라벨을 사용하는 것임
  - 하지만 이 휴리스틱을 한 번 정의하게 된다면 생산 시스템에서 휴리스틱을 사용을 해야할 것임
  - 만약 휴리스틱이 충분히 복잡하다면 ML을 그 시스템으로 바꾸는 것을 고려할만함
  - 하지만 휴리스틱을 재정의하는 것만큼 모델을 쉽게 재정의하고 나아가는 것은 조심하라
<img src="https://user-images.githubusercontent.com/32586985/72671457-cd74b900-3a8d-11ea-9e03-2f2f1d28bd7d.PNG">

- Causation
  - ML은 두 개 이상 혹은 그 이상의 상호관계나 상관관계를 정의할 수 있음
  - 하나의 이벤트 혹은 다른 것에 영향을 주는 원인 즉 원인을 정의하는 것은 매우 어려움
  - 무엇이 일어나는지 보는것은 쉽지만 그것이 왜 일어났는지 이해하는 것은 매우 어려움
  - 단순히 관측데이터만으로는 원인을 정의할 수 없음 / 다양한 부과적인 요소를 보면서 생각을 하게 됨 
  - 일반적으로 실험을 하고 원인을 규정짓는 등을 통해 바라봐야 함

- No Existing Data
  - 만일 모델을 훈련시킬 데이터가 없다면 ML은 효과가 없음
  - 데이터가 없다면 예시를 사용하고 휴리스틱과 규칙에 기반한 시스템을 만들 것이며 오직 유저와 상호작용한 데이터만을 가지고 있고 초기에는 학습 데이터 없이 휴리스틱 규칙만을 가지고 시작할 것임
  - 만일 학습 데이터가 있다면 그 안에 패턴을 찾아라 
  - 만일 패턴이 없거나 오직 초기에만 보이는 패턴만이 있다면 그 머신러닝은 효과가 크게 없을 것임
  - 만일 많은 패턴이 보이면 그것은 정확한 예측을 하는데 중요하며 머신러닝을 사용하는 것이 올바른 접근법이라는 것을 보여줌
